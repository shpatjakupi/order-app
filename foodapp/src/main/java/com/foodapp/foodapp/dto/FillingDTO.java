package com.foodapp.foodapp.dto;

import com.foodapp.foodapp.entity.Filling;

public class FillingDTO {
	private int id;
	private String name;
	private String description;
	private int price;

	public FillingDTO() {
		
	}
	
	public FillingDTO(Filling filling) {
		this.id = filling.getId();
		this.name = filling.getName();
		this.description = filling.getDescription();
		this.price = filling.getPrice();
	}

	public int getId() {
		return id;
	}

	public void setId(int id) {
		this.id = id;
	}

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}

	public String getDescription() {
		return description;
	}

	public void setDescription(String description) {
		this.description = description;
	}

	public int getPrice() {
		return price;
	}

	public void setPrice(int price) {
		this.price = price;
	}

	@Override
	public String toString() {
		return "FillingDTO [id=" + id + ", name=" + name + ", description=" + description + ", price=" + price + "]";
	}
}